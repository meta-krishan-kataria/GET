...
/*
	*//**
	...
	public List<Issue> getAllIssues(int deptId) 
	{
		...
		try 
		{
			...
			for(Issue i : issueList){
				System.out.println("Issue List::"+  i.getId()+ i.getTitle() );
			}
		}catch (HibernateException e) {
			e.printStackTrace();
		}finally{
	...
	@Override
	public List<Issue> getIssuesByStatus(int deptId, String status) 
	{
		List<Issue> issueList = null;
		Session session=null;
		...
		{
			session=sessionFactory.openSession();
			Query query=session.createQuery("from Issue where department.id= :deptId AND currentStatus= :status ") ;
			query.setInteger("deptId", deptId);
	...
	public List<Issue> getPendingIssues(int deptId) 
	{
		...
		{
			session=sessionFactory.openSession();
			Query query=session.createQuery("from Issue where department.id= :deptId AND currentStatus='alloted'") ;
			query.setInteger("deptId", deptId);
			issueList=query.list();
			//
			for(Issue i : issueList){
	...
	public boolean updateStatusOfIssue(int issueId, int deptId, String status) 
	{
		...
		try {
			...
			IssueStatus statusLog=new IssueStatus(issueId, status, new Timestamp(time), deptId);
	...
	public boolean setTargetResolutionDateForIssue(
		...
		try {
			...
			Query query=session.createQuery(
					"UPDATE Issue SET targetResolutionDate= :targetResolutionDate WHERE id= :issueId");
			query.setInteger("issueId", issueId);
			query.setTimestamp("targetResolutionDate", targetResolutionDate);
			
			System.out.println( targetResolutionDate.toString() );
	...
	public boolean addUserToDepartment( int deptId, int userId) 
	{
		...
		try {
			...
			//updating currentStatus in Issue 
			Query query=session.createQuery("UPDATE UserDetails SET department.id= :deptId, role= :'resolver' WHERE id= :userId");
			query.setInteger("deptId", deptId);
			...
			t.commit();
			session.close();
			System.out.println("User Added to department");
